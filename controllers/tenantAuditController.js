...
const getAllNonCompliances = async(req,res)=>{
    ...
    const noncomp = await pool.query('select noncompliances from new_audit where tenant_id = $1',[req.tenant.id])[0].noncompliances
    if (!noncomp){
...
const UpdateNoncompfromtenant = async(req,res)=>{
    //const ten_id = await pool.query('select tenant_id from tenant where email = $1',[req.body.tenant_email])[0].tenant_id
    const non_comp = await pool.query('update new_audit set noncompliances = $1 where tenant_id = $2 returning noncompliances',[req.body.noncompliances,req.tenant.id])[0].noncompliances
    if (!noncomp){
